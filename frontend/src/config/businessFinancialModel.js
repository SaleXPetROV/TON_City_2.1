...
// Уровень 1 = 1.0x, Уровень 10 = 3.5x
...
// Функция расчёта производства на конкретном уровне
export function getProductionAtLevel(businessType, level) {
  const baseProduction = BASE_PRODUCTION[businessType];
  ...
  for (const [resource, amount] of Object.entries(baseProduction)) {
    production[resource] = Math.round(amount * multiplier * 100) / 100;
  }
  
  return production;
}

// Функция расчёта требований на конкретном уровне
export function getRequirementsAtLevel(businessType, level) {
  const baseReq = BASE_REQUIREMENTS[businessType];
  if (!baseReq) return null;
  
  const multiplier = LEVEL_MULTIPLIERS[level] || 1;
  const requirements = {};
  
  for (const [resource, amount] of Object.entries(baseReq)) {
    requirements[resource] = Math.round(amount * multiplier * 100) / 100;
  }
  
  return requirements;
}

// Функция получения стоимости улучшения
export function getUpgradeCost(businessType, currentLevel) {
  const tier = BUSINESS_TIERS[businessType];
  const tierKey = `tier${tier}`;
  const costs = UPGRADE_COSTS[tierKey];
  
  if (!costs || !costs[currentLevel]) return null;
  
  return costs[currentLevel];
}

// Функция получения тира бизнеса
export function getBusinessTier(businessType) {
  return BUSINESS_TIERS[businessType] || 1;
}

// Экспорт всей модели
export const BUSINESS_FINANCIAL_MODEL = {
  BASE_REQUIREMENTS,
  BASE_PRODUCTION,
  LEVEL_MULTIPLIERS,
  UPGRADE_COSTS,
  ...
  RESOURCE_NAMES_RU: {
    ...
    parts: 'Запчасти',
    quartz: 'Кварц',
    ton_per_day: 'TON/день',
    rent_ton_per_day: 'Аренда TON/день',
    storage_bonus: 'Бонус хранения',
    logistics: 'Логистика',
    food_processed: 'Еда (готовая)',
...
export default BUSINESS_FINANCIAL_MODEL;
